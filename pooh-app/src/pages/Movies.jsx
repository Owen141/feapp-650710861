...
const GENRES = {...new Set(movies.map(m => m.genre))}; //ดึงgenreที่ไม่ซ้ำกัน
function Movies() {

  const [query, setQuery] = useState('');
  const [genre, setGenre] = useState('all');
  //const shown = movies.filler(m => m.title.toLowerCase().includes(query.toLowerCase));
  const q = query.trim().toLowerCase();
  const shown = movies.fileter(m => m.title.toLowerCase().split(' ').some(word => word.startsWith(q)) && (genre === 'all' || m.genre === genre));  // แยกคำแล้วตรวจสอบทีละคำ 
